...
#the pixel value for the image are ranging over [0,1], so for any 8bit gray scale images, it should be normalized to this scale range for training convenience.
...
def signal_motion_sample_v1(r,size, shift,ntime, dtype, **kwargs):
	...
	for i in range(ntime):
		...
		if reverseDice < reverseRate: d = np.add(d,bkg)
		else: d = np.subtract(bkg,d)
		data.append(np.clip(d,0, 1))
